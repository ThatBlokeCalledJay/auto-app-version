...
#### DONE:
  ...
  - [x] Fix bug introduced in v1.0.3. AAV wasn't commiting new version back to project file.
...
Don't worry about incrementing your app's version number every time you commit/integrate and deploy small patch updates. There's going to be a lot of them. Let AutoAppVersion increment the patch version for you. When you are ready to release a new Major or Minor version, simply update those version segments like normal in your csporj file. AutoAppVersion will set the patch segment back to 0 for you. This is really handy when your BuildPipeline automatically packs and pushes NuGet packages. No more "Package with same name and Version number already exists" because you forgot to update the version.  
...
2. Tell AutoAppVersion which version segment you want to automate (major, minor or patch). In this case we'll automate the patch segment. Replace the patch value (third value) with a $ symbol like so `<Version>1.0.$</Version>`. You have now created a mask.  
...
AutoAppVersion will also update your build's `VersionVariable` via the Azure DevOps Api. (This is why your DevOps PAT is required for authenticating the request). If you keep an eye on your `VersionVariable` you will see it automaticvally increment after each build or release. 
...
Set your version number in your project's csproj file. It is here you will be defining your version mask. Typically if you are planning on packaging your project, you will already have a `<PropertyGroup>` element which defines certain package information. It's the `<Version>` element we care about.
...
Incrementing higher priority segments will result in lower priority and masked segments being set back to 0.
